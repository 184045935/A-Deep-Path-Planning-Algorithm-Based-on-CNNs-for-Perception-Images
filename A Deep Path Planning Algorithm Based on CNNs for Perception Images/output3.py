...
# data preprocessing
img_data = np.zeros(5*100*100*3).reshape(5, 100, 100, 3)
for i in range(5):
    img = Image.open('test_pictures2/' + "test" + str(i) + ".bmp")
...
np.save('test_pictures2/pathplanning/img_data.npy', img_data)

# # label_data preprocessing
# data = []
# with open('test_pictures/testdata.pickle', 'rb') as f1:
#     for i in range(100):
#         d0 = []
...
#
# data = np.array(data)
# np.save("test_pictures/pathplanning/label.npy", data)

start = time.time()

data = np.load('test_pictures2/pathplanning/img_data.npy')
test_data = data[0:5]

# label = np.load('test_pictures/pathplanning/label.npy')
...
with tf.Session() as sess:
    ...
    for m in range(5):
        ...
        # print(output1)
        im = Image.open('test_pictures2/test' + str(m) + '.bmp')
        draw = ImageDraw.Draw(im)
        draw.line(output1, fill=(0, 0, 0))
        # draw.line(output2, fill=(255, 0, 0))
        # im.show()
        im.save('test_pictures2/pathplanning/labeled' + str(m) + '.bmp')
        del draw
